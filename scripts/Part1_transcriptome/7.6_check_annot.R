...
## Load annotation
newAnnot <- read.csv("/scratch/alicebalard/outData/assemblies/assemblyMergedFungi/annotation/assemblyMergedFungi_simplified_GOKegg.tsv", sep = "\t")                                                                                 
...
transc <- read.fasta("/scratch/alicebalard/outData/assemblies/assemblyMergedFungi/Trinity.fasta")

transc1 <- transc[names(transc) %in% eukTransc]

write.fasta(transc1,
            names(transc1),
            file.out= "/scratch/alicebalard/outData/assemblies/assemblyMergedFungi/Trinity_eukaryoteHits.fasta", nbchar=50000)
...
gtm <- read.table("/scratch/alicebalard/outData/assemblies/assemblyMergedFungi/Trinity.fasta.gene_trans_map") 

write.table(gtm[gtm$V2 %in% eukTransc,], "/scratch/alicebalard/outData/assemblies/assemblyMergedFungi/Trinity_eukaryoteHits.fasta.gene_trans_map", row.names=F, quote=F, col.names = F)

#################################################
## Subset annotation file for the Eukaryotic hits
write.table(newAnnot[newAnnot$blastxKingdom %in% "Eukaryota",], "/scratch/alicebalard/outData/assemblies/assemblyMergedFungi/annotation/assemblyMergedFungi_filterEuk_simplified_GOKegg.tsv", sep = "\t", quote = F, row.names=F, col.names=T)
